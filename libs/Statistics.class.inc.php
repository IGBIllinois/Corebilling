...
class Statistics 
{
	...
	public function getReservationUsage($startyear,$startmonth,$endyear,$endmonth){
		$userQuery = "SELECT u.user_name, u.id from users u left join reservation_info r on r.user_id=u.id where ((MONTH(r.start)>=:startmonth AND YEAR(r.start)=:startyear) OR YEAR(r.start)>:startyear) AND ((MONTH(r.start)<=:endmonth AND YEAR(r.start)=:endyear) OR YEAR(r.start)<:endyear) group by u.id";
